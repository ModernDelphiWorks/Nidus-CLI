...
procedure TNfeRouteHandler.Delete(Req: THorseRequest; Res: THorseResponse);
var
  LResult: TResultPair<String, Exception>;
begin
  LResult := GetNest4D.Get<TNfeController>.Delete;
  LResult.When(
...
procedure TNfeRouteHandler.Find(Req: THorseRequest; Res: THorseResponse);
var
  LResult: TResultPair<String, Exception>;
begin
  LResult := GetNest4D.Get<TNfeController>.Find;
  LResult.When(
...
procedure TNfeRouteHandler.Insert(Req: THorseRequest; Res: THorseResponse);
var
  LResult: TResultPair<String, Exception>;
begin
  LResult := GetNest4D.Get<TNfeController>.Insert(Req.Body);
  LResult.When(
    procedure (Msg: String)
    begin
      Res.Send(Msg).ContentType(CONTENTTYPE_JSON).Status(200);
...
procedure TNfeRouteHandler.Update(Req: THorseRequest; Res: THorseResponse);
var
  LResult: TResultPair<String, Exception>;
begin
  LResult := GetNest4D.Get<TNfeController>.Update(Req.Body);
  LResult.When(
    procedure (Msg: String)
    begin
      Res.Send(Msg).ContentType(CONTENTTYPE_JSON).Status(200);
